<script type="text/babel">
    ...
    import { LOAD_GOODS_CATEGORIES, LOAD_GOODS_LIST } from '../store';
    ...
    export default {
        ...
        computed: {
            ...mapState({
                categories (state) {
                    return state.mobile.store.goods_categories.datas;
                }
            })
        },
        methods: {
...
<style scoped>
    ...
    .goods-hd { display: -webkit-box; display: -ms-flexbox; display: flex; -webkit-box-align: center; -ms-flex-align: center; align-items: center; border-bottom: 1px solid #eee;}
    .goods-hd .title { -webkit-box-flex: 1; -ms-flex: 1; flex: 1; padding: 10px; }
    .goods-hd .icon { padding: 10px; }
    /* list */
    .goods-list .goods-item { display: -webkit-box; display: -ms-flexbox; display: flex; padding: 10px; border-bottom: 1px solid #ececec; }
    .goods-list .goods-item .item-left { width: 70px; height: 70px; margin-right: 10px; }
    .goods-list .goods-item .item-right { -webkit-box-flex: 1; -ms-flex: 1; flex: 1; font-size: 14px; }
    .goods-list .goods-item .item-right .goods-name { height: 40px; line-height: 20px; margin-bottom: 6px; white-space: normal; text-overflow: ellipsis; overflow: hidden; word-break: break-all; }
    .goods-list .goods-item .item-right .goods-info { height: 24px; line-height: 24px; position: relative; overflow: hidden; }
    .goods-list .goods-item .item-right .goods-info .goods-price { position: absolute; color: #ff6700; }
    .goods-list .goods-item .item-right .goods-info .mint-button { height: 24px; border-color: #ff6700; font-size: 12px; float: right; }
    /* large */
    .goods-large { display: -webkit-box; display: -ms-flexbox; display: flex; -webkit-flex-wrap: wrap; -ms-flex-wrap: wrap; flex-wrap: wrap; padding: 5px; }
    .goods-large .goods-item { padding: 5px; width: 50%; line-height: 20px; font-size: 12px; }
    .goods-large .goods-item .item-right .goods-name { margin-top: 5px; word-break: break-all; }
    .goods-large .goods-item .item-right .goods-info .goods-price { color: #ff6700; }
    ...
    <div id="GoodsList">
        <div class="page-infinite-wrapper" ref="wrapper" :style="{ height: wrapperHeight + 'px' }">
            <div class="goods-hd">
                ...
                <div class="icon" @click="cut = !cut">
                    <i class="fa" :class="[cut ? 'fa-th-large' : 'fa-th-list']" aria-hidden="true"></i>
                    <i class="fa" aria-hidden="true"></i>
                </div>
                <div class="icon" @click="siftGoodsPopupVisible = !siftGoodsPopupVisible"><i class="fa fa-filter" aria-hidden="true"></i></div>
            </div>
            <ul :class="[cut ? 'goods-large' : 'goods-list']" v-infinite-scroll="" infinite-scroll-disabled="loading" infinite-scroll-distance="50">
                <li class="goods-item" v-for="(list, index) in goods_list" @click="selectGoods(list)">
                    ...
                    <div class="item-right">
                        ...
                        <div class="goods-info">
                            <span class="goods-price">增减价: {{ '￥' + (list.goods_floatingprice > 0 ? '+' + list.goods_floatingprice : list.goods_floatingprice) }}</span>
                        </div>
                    </div>
                </li>
            </ul>
                ...
                <div class="content" v-if="categories">
                    <div class="sift-row">
                        ...
                        <div class="row-body" :class="{ expand: cateMore_1 }">
                            <span v-for="(cate, index) in categories.categories" :class="{on: cate.gc_id == search_list.cate_id1}" @click="selectCategories(cate, 1)">{{ cate.gc_name }}</span>
                        </div>
                    </div>
        ...
        <mt-popup v-if="goodsViewPopupVisible" v-model="goodsViewPopupVisible" position="bottom" class="goods-view-popup">
            <goods-view :goodsId="goods_id" @selectgoodsspec="selectGoodsSpec"></goods-view>
        </mt-popup>
    </div>
</template>